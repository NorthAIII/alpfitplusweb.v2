...
- `_dev/ILKELER.md` — **varsa** oku (`prd` ya da `map-codebase` oluşturmuş olabilir; modül/faz yapısını ufuk/öncelik ilkelerine göre öner). PRD'siz greenfield'da henüz yoktur (kickoff-docs'ta doğar) — **yoksa** atla; varlık kontrolü esas.

### Re-Kickoff Modunda (protokol çekirdek dokümanları zaten okudu)
- `_dev/PRD/` altındaki tüm dokümanları oku
- `_dev/MODULE-MAP.md`, `_dev/PHASES.md`
- Etkilenen modül dokümanları (`_dev/modules/`)
- `_dev/ILKELER.md` — proje ilkeleri (mimari/faz güncellemesini yönlendirir); **yoksa** (eski kurulum) `—` işaretle ve kullanıcıya bildir — kickoff-docs oluşturacak
...
- Kullanıcı PRD yapmak istemezse mevcut kickoff davranışıyla devam et (projeyi sıfırdan anlama):
  ...
  - Proje ilkeleri — kullanıcının kendi düşüncelerini **sor**, kafana göre üretme: proje ufku, en yüksek öncelikli eksenler, pazarlık konusu olmayanlar. Proje hakkında öğrendiklerinle harmanla; cevaplanmayan/ertelenen alan boş kalır. (Cevaplar KICKOFF-NOTES'a; ILKELER.md yoksa kickoff-docs'ta bu cevaplardan doğar. Dosya varsa ve alanları doluysa yeniden sorma — teyit et.)
...
Tüm kararları (Adım 1'deki ilke cevapları dahil) `_dev/KICKOFF-NOTES.md` dosyasına yaz (`_dev/` klasörü yoksa oluştur). Bu dosya kickoff-docs oturumunda okunacak.

...
`_dev/` ve CLAUDE.md varsa CLAUDE.md'deki Oturum Başlangıç Protokolü'nü uygula (çekirdek dokümanlar orada listelidir — eksik dosyalar atlanır). İlk PRD oturumu senaryosunda (`_dev/` yok) bu protokol uygulanmaz; doğrudan aşağıdaki kontrollere geç.

### Başlangıçta Kontrol Et
- `_dev/PRD/` klasörünün varlığını kontrol et
- `_dev/` klasörünün varlığını kontrol et
- `_dev/ILKELER.md` varlığını kontrol et — **varsa oku** (Q&A'yi yönlendirir; "Bu Projeye Özgü" alanları boşsa Adım 3'te kullanıcıya sorulacak); yoksa Adım 3 başında oluşturulacak
...
**Adım 0 — Protokol & Okuma Onayı (her şeyden önce):** Yukarıdaki "Okunacak Dosyalar"ı uygula (ilk PRD oturumunda `_dev/` yoksa protokol atlanır — yokluk hata değil), sonra tek satırlık okuma-onayını yaz (kural → CLAUDE.md: "Protokol ve `/devflow:` Komutları Arasındaki İlişki" → Okuma onayı). Onay yazılmadan başlama; yazınca da durma — Adım 1'e geç.
...
**Keşfe başlamadan ILKELER.md'yi hazırla:** `_dev/ILKELER.md` yoksa template'ten (`.claude/commands/devflow/templates/ILKELER.md`) evrensel tabanla oluştur (`_dev/` kökünde, PRD/ altında değil; taban evrenseldir, otonom basılır). Böylece keşif boyunca ilkeler öner+onayla'yı ilk sorudan itibaren yönlendirebilir.

**Projeye-özgü ilkeleri kafana göre üretme — kullanıcıya sor:** "Bu Projeye Özgü" alanları boşsa (dosya yeni doğdu ya da map-codebase boş bırakmış), keşfin doğal bir noktasında ufuk / öncelikli eksenler / pazarlık-konusu-olmayanlar üçlüsünü kullanıcının kendi düşünceleriyle aç; proje hakkında öğrendiklerinle kullanıcının cevaplarını harmanla — yazım Adım 4'te. Cevaplanmayan/ertelenen alan boş kalır.
...
**ILKELER.md'yi tamamla (`_dev/ILKELER.md`, `_dev/` kökünde):**
- Dosya Adım 3 başında evrensel tabanla oluşturuldu. Burada Adım 3'te sorulan ilke cevaplarını ve keşifte belirginleşenleri ("Bu Projeye Özgü": proje ufku, en yüksek öncelikli eksenler, pazarlık konusu olmayanlar) proje bilginle harmanlayıp yaz. Sorulmamış/cevaplanmamış alanları boş bırak — uydurma. (Evrensel tabanda projeye uymayan varsa çıkar/uyarla.)
...
**Sabit dosyalar:**
- `VERSIONS.md` — Feature-versiyon haritası tablosu (hangi feature hangi versiyonda). Dosyanın başına şu KURAL yorumu birebir yazılır (üretilen dokümanla birlikte yaşar):
  ```
  <!-- KURAL: Bu harita PLANDIR, durum panosu değil — durum işareti/sütunu tutulmaz
       (feature durumu MODULE-MAP'te, aktif versiyon DURUM.md'de). İçeriği PRD
       oturumları yazar; gerçekle uzlaştırmanın tek sahibi prd-review'dur; faz
       döngüsü bu dosyaya dokunmaz. -->
...
**Versiyonlar:**
Büyük projelerde `versions/` klasörü + detay dosyaları. Küçük projelerde VERSIONS.md tek başına yeterli. **Detay dosyaları da plandır, canlı uygulama panosu değil** — VERSIONS.md'nin başına yazılan KURAL onlar için de geçerlidir: faz döngüsü bu dosyalara dokunmaz, tur-tur durum/ilerleme oraya yazılmaz. Versiyon sonunda VERSIONS.md'de kapanan satırların arşiv-mezuniyet hedefi ise yine burasıdır (CLAUDE.md → Boyut ve Bölünme) — mezun satır kapanmış bir kaydın arşividir, canlı pano değil.
...
Tüm PRD dokümanlarını ve `_dev/ILKELER.md`'yi commit & push yap:
```
...
- ILKELER.md PRD akışında ilk prd oturumunda doğar ve Q&A'yi yönlendirir; projeye-özgü alanları kullanıcıya sorularak dolar; sonraki prd adımlarında (prd-refine, prd-review) güncellemeye açıktır. (Tüm doğum yolları → ILKELER template'i, "Bu Doküman Hakkında")
...
**İzsiz mezuniyet:** Bir bilgi olgunlaşıp PRD dokümanlarına (feature dosyası, VERSIONS.md, esnek içerik dosyası vb.) aktarıldıysa SESSION-NOTES'tan **tamamen silinir** — "X'e aktarıldı", "✓ tamamlandı" veya benzeri hiçbir iz/breadcrumb bırakılmaz. SESSION-NOTES'a bakan biri o bilginin bir zamanlar burada olduğunu bilmek zorunda değil; tek kaynak hedef dokümandır. Aynı şekilde artık gerekli olmayan bilgiler (kullanıcı tarafından reddedilen fikirler, güncelliğini yitirmiş gözlemler) da doğrudan silinir. Dosya uzuyorsa bu çoğunlukla mezuniyetin yeterince uygulanmadığının sinyalidir (istisna: yukarıdaki erken/geniş keşif dönemi) — olgunlaşmış bilgileri tespit et, PRD dokümanlarına aktar ve SESSION-NOTES'tan izsiz çıkar.

...
- `_dev/KICKOFF-NOTES.md` **varsa** (ilk kontrol — DURUM/CLAUDE.md durumundan bağımsız) → kickoff/re-kickoff konuşması bitmiş, aktarım bekliyor (NOTES ancak kickoff-docs aktarımı bitince silinir). Kullanıcıya bildir: `/devflow:kickoff-docs` ile devam et. **Dur.**
...
- DURUM var ama **kökte** `CLAUDE.md` yoksa → "yok" demeden önce `.claude/CLAUDE.md`'ye bak (filoda ölçülmüş hâl). Dosya varsa ve `## Oturum Başlangıç Protokolü` başlığını içeriyorsa projenin **kendi parent'ıdır** (`kickoff-verify` Adım 3'ün testi) — harness onu zaten bağlama yükler, doktrin bu oturumda mevcuttur: **durma**, tek satırla bildir ("parent `.claude/CLAUDE.md`'de; yerini denetlemek istersen `/devflow:kickoff-verify`") ve Adım 2'ye geç. **İkisi de yoksa** — ya da `.claude/CLAUDE.md` o başlığı içermiyorsa (o bir global kural kopyasıdır, ilgisizdir) — kurulum yarım kalmıştır: kullanıcıya bildir, `/devflow:kickoff-verify` Adım 3 önce parent'ın yerini tespit eder. **Dur.**
...
- **Adım = `plan`** iken Task Durumu tablosunda ✅ ve/veya kesilen 🔄 task'lar görmek çelişki DEĞİLDİR — plan revizyonu durumudur (run-task revizyon gerektiğinde Adım'ı `plan`'a çeker); normal şekilde `plan-phase`'e git.
...
- **Adım boş ama yukarıdakine uymuyor** (ör. versiyon geçişi sonrası: Aktif Faz/Adım boş, Versiyon Sonu Durumu `içerik_fazları`) → versiyon-geçişi sınırı, `next`'in kapsamı dışı. **Çalıştırma — durumu raporla ve kullanıcıya sor.** Bu ayak izi birden fazla anlama gelebilir: yeni versiyonun ilk `discuss-phase`'i başlatılacak, re-kickoff bekleniyor ya da harici teslim/go-live beklemede — hangisi olduğunu kullanıcı bilir.
...
- DURUM sıradaki adımın tek yetkili kaynağıdır; ama tablo ile çelişki görürsen (ör. Adım=`task` ama tüm tasklar ✅) tabloyu esas al ve doğru adıma git. (İstisna: Adım=`plan` + tamamlanmış/kesilen task'lar çelişki değildir — plan revizyonu; bkz. Özel durumlar.)

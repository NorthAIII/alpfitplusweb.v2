...
10. **Hiçbir dosya yarım veya atlanarak okunmaz.** Bir Read çağrısı dosyayı tam getirmezse — çıktıda truncate / **PARTIAL** / satır-limiti uyarısı görürsen ya da istediğin aralık eksik döndüyse — **kör deneme-yanılma yapma** (giderek daralan aralıkları rastgele deneme) ve **yarım okuyup sonraki işe geçme**. Kural `_dev/` ile sınırlı değil; kod ve dış dosyalar dahil her Read için geçerli. Sırayla:
    ...
    4. **`_dev/` dokümanıysa işaretle:** Tek-okumaya sığmayan bir `_dev/` dokümanı aynı zamanda bir Boyut ve Bölünme ihlalidir — ama **bu oturumun işi o değildir**: soruya çevirme, müdahaleye de girişme. Teşhisi (temizlik / bölme / supap-mezuniyeti — `DOKUMAN-DISIPLINI.md` → Boyut ve Bölünme) tek satırla oturum kapanış bloğunun «Açık kalemler» satırına `önerilir: /devflow:audit-docs — <doküman> tek okumaya sığmıyor` biçiminde yaz; dokümanın kendi turu oradadır ve kulvarı orada `Onay Ölçütü` belirler. **İstisna:** zaten o dokümanın kendi boyut kapısındaysan (research/verify/review Adım 4b/6b/5b, `audit-docs`) iş o adımın kendisidir — orada çöz. (Kod/dış dosyalarda bu adım yok — yalnız kurtar.)
...
12. **Gördüğün sorunu düşürme.** Kapsam dışı bir sorun/uyarı (CI failure, bot uyarısı, bug) fark edersen işini kesme, kapsama alınmadıkça o an çözmeye de girişme — (**Ayrım:** bu kural *senin gördüğün* ve henüz ne yapılacağı kararlaşmamış sorun içindir. Kullanıcı bir işi karara bağlayıp sonraya bıraktıysa — "bunu da yapalım ama sonra" — ortada triyaj bekleyen gözlem değil **ertelenmiş iş** vardır; evi kanvas değil `⬜ Bekliyor` bir QUICK kaydıdır → CLAUDE.md → Oturum Kapanışı → **Ön-hazırlık**.) `_dev/BULGULAR.md` → **Gelen Kutusu**'na kaynak işaretli tek satır düş (`- [TASK-X.YY] ...`; işaret: `[TASK-X.YY]` / `[PHASE-N]` / `[QUICK-NNN]` / `[oturum türü]`. Dosya yoksa template'ten oluştur — `.claude/commands/devflow/templates/BULGULAR.md` — ve INDEX'e kaydet). Secret/credential değerini kayda asla yazma — yalnız konumunu. PRD/vizyon düzeyi fikir buraya değil → `/devflow:prd-note`. Bu kutuyu verify-phase Adım 1 süpürür (bu faza dokunanlar), audit-product uzlaştırması triyaj eder.
13. **Kullanıcının diliyle konuş.** Kullanıcıya sorduğun soru ya da sunduğun rapor **pratik karşılığıyla** kurulur: ne değişecek, ne kaybolacak/kalacak, hangi iş nasıl görünecek. İki ayağı vardır:
    - **Teknik jargon.** Kullanıcı çok farklı alanlarda proje yürütür; her alanın sözlüğüne hakim olmayabilir ama her konunun pratikte nasıl görüneceğine dair tercih sahibidir. Soruyu terimle değil davranışla kur; terim gerekiyorsa parantez içinde açıkla. ❌ "idempotency mi optimistic locking mi?" → ✅ "aynı butona iki kez basılırsa iki kayıt mı oluşsun, ikincisi sessizce yok mu sayılsın?"
    - **Doküman iç-referansı.** `_dev/` dokümanları ve DevFlow template'leri **Claude'un çalışma belleğidir**; kullanıcının onları okuduğunu varsayma. Doküman adı, bölüm/alan adı, satır numarası, template yolu ve motor terimi (`drift`, `conformance`, `exclude`, `parent`, `atomizasyon`) bir soruyu **kuramaz** — en fazla kalemin sonunda izlenebilirlik için durur. ❌ "MODULE.md:15'e göre F2.1 bloğunda Kabul Kriterleri alanı yok, ekleyelim mi?" → ✅ "Ödeme özelliğinin 'ne zaman bitmiş sayılır' ölçütleri açıklama metnine karışmış; ayrı başlık altına alıyorum."
    - **Ayrım:** `TASK-4.02` · `PHASE-3` · `QUICK-135` · `B-017` **iç-referans değildir** — bunlar kullanıcının kendi iş birimleridir, adıyla anılır. Ürün gerçekliğini gösteren çapa da kalır (`docker-compose.yml:11`, konsol çıktısı, repro adımı) — o kanıttır, koordinat değil.
    - **Ölçüt:** kullanıcı hiçbir dokümanı açmadan karar verebiliyorsa soru doğru kurulmuştur. Model örnekler: `verify-phase` Adım 4 (test modu sorusu) ve Adım 5a (manuel test yönergesi).

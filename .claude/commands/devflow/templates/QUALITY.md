...
- Uygulama katmanındaki güvenlik kapıları canlı serving zinciri (reverse proxy, CDN, cache, edge) üzerinden de etkin mi — önündeki katman kapıyı route-dışı bırakabilir, header'ını soyabilir, cache'ten baypas ettirebilir mi; origin'e doğrudan erişimle atlatılabilir mi? (Serving katmanı olmayan projede bu madde çıkarılır.)

...
**Sıfırlamanın sahibi devralan ya da kapatan oturumdur, komutun adı değil** — `resume` bunun yalnız bir taşıyıcısıdır; kardeşlerini burada sayma, hükmü kendi akışında uygulayan komut zaten uygular. Atlanırsa tetik depoda ayakta kalır ve sonraki oturum — task ✅'lanıp arşive taşınmış olsa bile — yine `/devflow:resume`'a yollanır. Orkestratörlü koşumda bedel **koşumun kulvarına** göre değişir (`run-phase` → `Kulvar` / `Kulvar — QUICK` / `Kulvar — BULGULAR`, Duraklatma Notu dalı) ve üç kulvarda üç ayrı hâl alır: FAZ kulvarında `task çalıştırma` / `review` değerinde bir tur `resume`'a gider ve ilerleme üretmez, `quick` ve tanınmayan değerlerde koşum **hiç tur açmaz** (ölçüt 13); QUICK kulvarında dal **tersine döner** — `quick` kulvar içidir ve `resume` dağıtılır, `task çalıştırma` / `review` kulvar dışıdır ve koşum durur; **BULGULAR kulvarında dolu bir not, değeri ne olursa olsun koşumu durdurur** (kuyruğu bulgulardır ve `resume` argüman almaz, yani dağıtılırsa ajan kuyruğun dışındaki bir işin üstüne yollanır).

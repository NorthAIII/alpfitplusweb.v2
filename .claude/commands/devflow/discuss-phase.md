...
- `_dev/BULGULAR.md` + seçilen bulgu atomları → bulgu fazı isteniyorsa oku; Sıradaki Fazlar boşsa açık bulgu var mı diye kısaca bak (Adım 1)
...
**Her şeyden önce — Protokol & Okuma Onayı:** Aşağıdaki adımlara (0 dahil) geçmeden önce yukarıdaki "Okunacak Dosyalar"ı oku ve tek satırlık okuma-onayını yaz (kural → CLAUDE.md: "Protokol ve `/devflow:` Komutları Arasındaki İlişki" → Okuma onayı). Onay yazılmadan başlama; yazınca da durma — Adım 0'a geç.
...
**Bulgu fazı:** Kullanıcı fazı bulgulardan doldurmak isterse — ya da Sıradaki Fazlar boşken `_dev/BULGULAR.md`'de açık bulgu varsa bunu seçenek olarak hatırlat — konu kaynağı BULGULAR kanvasıdır: index'in öncelik sırasıyla **işaretsiz** bulgu atomlarını oku (`→ Faz N`/`→ TASK-X.YY` işaretliler zaten rotada — atla), **faz dolunca dur** (dar faz ilkesi; kalanlar kanvasta sırasını korur, sonraki bulgu fazını bekler). Bulgu içeriği faz kapsamına Adım 6'da işlenir; kanvas işaretlemesi de orada yapılır.
...
3. Genel bilgileri (amaç, milestone, feature listesi) **ve ilk satırdaki `**Durum:**` alanını (`🔄 Devam ediyor`)** doldur — alan PHASES'teki faz durumuyla aynı değeri taşır; kapanışta review-phase ✅, erken sonlandırmada prd-review ⚠️ yazar (template KURAL'ı)

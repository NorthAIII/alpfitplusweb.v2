...
| Komut | Açıklama |
|-------|----------|
| `/devflow:run-phase [tur talimatı]` | Fazın icra kısmını (task → verify → review) **ardışık alt oturumlarla** yürütür: her adım kendi alt ajanında koşar, bu oturum onları sırayla açar, dönüşlerini ölçer ve yalnız gerçek soruları sana getirir. Kulvar dışına çıkınca durur (fazın kapanışı da budur) |
| `/devflow:quick [QUICK-NNN \| ne yapılacağı]` | Ad-hoc task — faz döngüsü dışı hızlı iş. İşi doğrudan yazabilirsin (`/devflow:quick login yönlendirmesi bozuk`); yarım kalan **ya da önceki bir oturumun hazırladığı** bir iş varsa numarasıyla çağrılır (`/devflow:quick QUICK-135`); boş **ya da serbest metinli** çağrıda önce bu kayıtlar taranıp sorulur. Projede bir yayın hattı beyanlıysa **yayın** ve **acil düzeltme** de bu komutun türleridir (akış: `_dev/GIT-STRATEJI.md`) |

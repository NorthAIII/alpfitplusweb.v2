...
- **Yasal metnin artık bir test kapısı var** (TASK-2.18): `tests/legal-consistency.test.ts` yayındaki sekiz beyanı depo içi dayanaklarına bağlar — sorgu hariç tutma (`data-exclude-search`), ölçüm yükü (`track()`), ürün görseli temizliği, deponun yalnız yazma ile kullanılması, ölçümün üçüncü tarafa gitmemesi, çerez yokluğu, başvuru adresinin `CONTACT.support`'tan gelmesi, form ucu için bölge sabitlenmemiş olması. Metin **kopyalanmaz**: her dal cümleden kısa bir parça alır ve o parçanın **tam bir kez** geçtiğini ölçer — cümle yeniden yazılırsa kapı kırılır ve bu bilinçlidir ("bu beyanın dayanağı hâlâ duruyor mu?"). ⚠️ Kapının dışında kalan iki olgu kendi `ÖLÇÜLEMEYEN` yorum bloklarında yazılı: depo **anahtarının yetki yüzeyi** komşu depodadır (`../Alpfitplus-website.v1/pocketbase`), başvuru adresinin **posta alması** DNS olgusudur (TASK-2.20). "12 ay" saklama dalı TASK-2.19'undur.

...
- **`UMAMI_USERNAME`** / **`UMAMI_PASSWORD`** — `umami.kiwiailab.com` panel hesabı (`admin`; kurulumdaki tek hesap). Umami 3.1.0'da API anahtarı olmadığı için tek kimlik yolu budur → `POST /api/auth/login` → Bearer token. **Çalışıyor** (TASK-1.07, 2026-09-22: giriş 200, rol `admin`, site kaydı API ile açıldı). ⚠️ **Bu parola kaybedilirse panele giriş kalıcı olarak kaybolur** — Umami 3.1.0 sıfırlama aracı taşımıyor (detay: [Kendi sunucu atomu](kendi-sunucu-n8n-bunker-umami.md) → Umami).

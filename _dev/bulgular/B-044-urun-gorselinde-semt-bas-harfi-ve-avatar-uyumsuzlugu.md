...
**TASK-2.15 (2026-09-23) — kalem 3'ün İDDİA yarısı kapandı; kalem 1 ve 2 aynen açık.**

Bu atomun Koruma Önerisi'nin (b) maddesi uygulandı: *"iddia sözlüğü eklenir (`₺…B/ay`, `+%NN`, `en hızlı`, `en yüksek`, `rekor`, `Şubede 1.` …)"*. Sözlük `research/lib/claim-leak.mjs`'te tek kaynak; `auditTexts` dört dallı.

**Kalem 3 rakamla ölçüldü.** Bu atomun ve B-018'in adıyla saydığı **20** sızıntı dizgesi iki denetime karşı koşuldu: eski **3/20**, yeni **10/20**. ⚠️ Bu atomun metni "21 dizge / 20 kör" diyor ama kod bloğu 19 kör + 1 görülen = **20** yazıyor; 21'inci dizge hiçbir yerde yazılı değil — ölçüm yazılı olanlar üzerinden yapıldı. Ayrıca "tek GÖRÜR: Alpfit Plus" ifadesi bugün **tarihsel**: TASK-2.14 hedef tarafını çıkardığı için o dizge artık bulgu değil (doğrusu bu), buna karşılık `Beşiktaş` · `Simge & Gizem` · `Gizem Ö.` ad dalınca görülüyor.

**Envanter tablosu tazelendi** (hat yeniden koşuldu, dört çıktı gözle okundu):

- **`cockpit.webp`** — satırın iddia kalemlerinin tamamı düştü: `+%12,4` · `+%8/+%11/+%34 geçen aya göre` · `en hızlı` rozeti · `1. ciro`/`2. ciro` · `Büyüme (MoM)` satırı · `en iyi = ●` · ve klibin altındaki **"Patron özeti"** kartı (`sube.webp`'in düşürülme gerekçesiyle aynı sınıf: *"…lider … %34 büyümeyle en hızlısı — 4 ayda 227 üyeye ulaştı … optimize edilirse hedef %82'ye en yakın aday"*). Satırda **açık kalan:** `842 Aktif Üye · +71 bu ay yeni` (nötr gösterge, bilinçle serbest), `Vadi · Alpfit Plus · 4 aylık` ve `3 şube · Haziran 2026` (tarih/makullük sınıfı — aşağıda).
- **`finans.webp`** — `+%12,4` ve `geçen aya göre` düştü. Açık: `%59/%29/%12 pay`, `₺2.140.000`, `Ciro Trendi · son 6 ay`, `Haziran 2026` (hepsi nötr ya da tarih sınıfı).
- **`antrenor.webp`** — **`★ Şubede 1.` düştü** (bu atomun adıyla saydığı dizge) ve `+12 geçen aya göre` düştü. Açık: **`Ekipte: Mar 2023`**.
- **`raporlar.webp`** — "Antrenör Performansı" şablonunun açıklama metni `REPLACEMENTS` sınıfı bir eşlemeyle düzeltildi (aşağıdaki "yeni gözlem" kalemi **kapandı**): *"Ders sayısı, ciro, doluluk ve öğrenci tutma"* → *"Ders sayısı ve prim verisi"*. Yerine yazılan metin ayrıca ölçüldü — ürünün kendi servisi (`trainer-performance.service.ts`) performansı "birim ders ücreti × verilen ders sayısı" diye tanımlıyor, kendini "prim metriği" olarak adlandırıyor ve şube atfı gerçek.

**Açık kalan kalemler ve neden:**
- **Kalem 1 (semt baş harfi) ve kalem 2 (`.av` sınıfının 25 düğümü) hiç etkilenmedi** — `AVATAR_SELECTOR`/`INITIALS` konusu, bu fazın kapsam dışı listesinde. Yedi baş harfi dizgesi (`BŞ` · `TU` · `KA` · `DK` · `EÖ` · `BT` · `AK`) yeni denetimde de kör; iddia sözlüğünün sınıfı değiller.
- **Tarih/makullük sınıfı — KARARA BAĞLANDI ve bu atomdan düştü** (Kıvanç, audit-product triyajı 2026-09-23): temizlenmeyecek. Ölçüm soruyu küçültmüştü — bu atomun saydığı üç dizeden ikisi (`Açılış: Şubat 2026 · 4 aylık` · `Aktif · Alpfit Plus konsepti`) **şube ekranıyla birlikte yayından çıktı** (QUICK-001; `/product/sube.webp` 404), geriye yalnız `antrenor.webp`'teki `Ekipte: Mar 2023` kaldı ve o kurgusal kulübün geçmişi hakkında, ürünün kullanım süresi hakkında değil. Kalıpla kapatma ölçümle çürütüldü (`son N ay` meşru rapor yüzeylerini kırmızıya düşürüyor). Kayıt: `BULGULAR.md` → Bilinçli Tercihler. **Bu atomun açık kalemleri artık ikidir:** avatar baş harfleri (kalem 1-2) ve Hero'nun elle yazılmış `%78`'i (evi M6 F6.4).
- **Hero'daki elle yazılmış `%78` kartı** aynen açık — hattın çıktısı olmadığı için `auditTexts` onu yapısal olarak göremez; evi M6 F6.4.

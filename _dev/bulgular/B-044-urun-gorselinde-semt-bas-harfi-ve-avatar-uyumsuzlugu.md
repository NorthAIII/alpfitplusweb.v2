...
- **Tarih/makullük sınıfı bilinçle alınmadı.** Bu atomun önerdiği `son N ay` · `Açılış: …` · `Ekipte: …` kalıpları sözlüğe **girmedi**: CLAIMS'in "Söylenemez" sütununda karşılıkları yok ve ölçüldü ki kalıp alınsaydı finans ekranının `Ciro Trendi · son 6 ay` ekseni ile üç ekrandaki `Haziran 2026` başlığı kırmızıya düşerdi — üçü de ürünün meşru rapor yüzeyi. Bu sınıf Gelen Kutusu'ndaki `[audit-product SORU]` satırıyla **kullanıcı kararı** bekliyor.

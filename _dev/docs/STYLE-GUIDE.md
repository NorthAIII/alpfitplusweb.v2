...
**Reddedilen kalıbın ÖLÇÜSÜ üç mekanik ayaktır — "güzel oldu" ölçüm değildir** (TASK-3.18'de kuruldu, TASK-3.19'da tekrarlandı; B-051'in kapanış ölçütü). Yukarıdaki *"jenerik ikonlu kart ızgarası"* bir estetik yargıdır ve doğrudan bir kapıya bağlanamaz; ama tarifi sayılabilir üç parçaya bölünür ve bir bölümün kalıptan çıkıp çıkmadığı böyle **ölçülür**: **(1) ızgara kabı** — `display:grid` olan ve ≥ 4 çocuk taşıyan kap (sütun sayısıyla birlikte); **(2) ikon karosu** — kare (±2 px), ≥ 32 px, zemini ya da halkası olan ve içinde `svg` bulunan kutu (`IconBox` bunun ta kendisidir); **(3) kalem kutusu** — bölümdeki tekrar eden kutuların **benzersiz ölçü sayısı** (hepsi birebir aynıysa 1). Ölçüm yayın kopyasına (3100) karşı, 320/390/1440'ta koşulur ve **kapsam dışı bırakılan parçalar ayrı raporlanır** — `Modules`'ün 5'li ikon şeridi kullanıcı kararıyla duruyor, yani bölümde kalan 1 ızgara kabı ve 5 karo onundur. Ölçülen kapanışlar: `Benefits` 1 kap/8 karo/1 ölçü → **0 / 0 / 2**; `Modules` öne çıkanlar 1 kap/5 karo/2 ölçü → **0 / 0 / 3**.

**"Her bölüm bir öncekinden farklı ritimde" maddesi SAYILARAK uygulanır** (aynı turlar): yeni bir bölüm düzeni seçmeden önce sayfanın bugünkü ritim envanteri çıkarılır, çünkü bir kalıp "bize yeni" olabilir ama sayfada zaten sekiz kez kullanılıyor olabilir. Ana sayfanın 15 bölümünde bugün: **iki sütunlu bölme 8** (Hero · Chaos · Roles · ProductStory · HowItWorks · PricingBlock · FounderProgram · Faq) · **eşit kartlı ızgara 2** (WhyUs · SegmentsGrid) · **tam genişlikte saç teli döküm 1** (Benefits, TASK-3.18) · **çerçeveli eşit-olmayan hücreli pano 1** (Modules, TASK-3.19) · kayan şerit · akış şeması · ortalanmış bant. Ölçüt komşuluk değil **tekrar**: ardışık iki bölüm aynı ritmi kullanmaz, ve sayfada zaten çoğunlukta olan bir ritmi bir kez daha eklemek "çeşitlilik" sayılmaz. Aynı disiplinin ikinci yüzü: bir önceki turun **jesti** de tekrarlanmaz — Faydalar'da ikon satır içi küçük bir işarete indiği için Modüller panosunda ikon hiç kullanılmadı.
...
**Son Güncelleme:** 2026-09-25 — TASK-3.19: reddedilen kalıbın **ölçüsü** yazıldı (üç mekanik ayak: ızgara kabı · ikon karosu · kalem kutusunun benzersiz ölçü sayısı; kapsam dışı parçalar ayrı raporlanır) ve *"her bölüm bir öncekinden farklı ritimde"* maddesinin nasıl **sayılarak** uygulandığı — ana sayfanın bugünkü ritim envanteri, ve bir önceki turun jestinin de tekrarlanmadığı. B-051'in "koruma önerisi"nin metin ayağı budur; mekanik kapı kurulmadı (kalıp estetik yargı). **Kullanıcının refleks listesine, token değerlerine ve tipografiye dokunulmadı.** — 44 px'i hangi yolla sağlayacağın elemanın `display`'ine bağlıdır (görünür kutuyu büyüt / satır içinde yalnız dolgu / dolgu + negatif margin telafisi), görünmez genişletmenin kendi fail-open'ı kayda geçti (komşu hedeflerle çakışmayı kapı görmez; iki gerçek örnek ölçülüp düzeltildi) ve onay kutusunun `appearance: none` gerekçesi + hata/odak deyiminin nasıl korunduğu yazıldı. Token değerlerine, tipografiye ve refleks listesine dokunulmadı.

...
**Son Güncelleme:** 2026-09-24 — TASK-3.15: iki yeni kural yazıldı — (1) kaydırılabilir şeritte kart tavanı **kabın görünür genişliğine** bağlanır (pencereye değil) ve sonraki karttan bir parça görünecek kadar pay taşır; `100%` tavan kapıyı yeşile çevirirken şeridin kaydırılabilir olduğunu görünmez kılıyor (ölçüldü). (2) Ürün ekranının telefon/tarayıcı çerçevesi rolün `device` beyanından değil **görselin kendi oranından** türer — beyanı değiştirmek siteye yanlış cümle söyletirdi. Düzen Tuzakları'na bir satır, gerekçeler nesir olarak. Token değerlerine ve refleks listesine dokunulmadı.

...
**Son Güncelleme:** 2026-09-24 — TASK-3.14: `whitespace-nowrap`'in yeni sınırı yazıldı (`Button` tabanında `sm:` ile sınırlı) — Düzen Tuzakları'na kendi satırı, gerekçesi ve ölçümleri nesir olarak. Kırpmayı bitirenin `min-w-0` değil `nowrap`'in kalkması olduğu izolasyonla ölçüldü; `sm:` sınırı iki adayın 16 rota × 6 genişlik karşılaştırmasından çıktı. Token değerlerine ve refleks listesine dokunulmadı.

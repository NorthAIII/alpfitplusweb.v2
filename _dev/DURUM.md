...
**Son Güncelleme:** 2026-09-23 — TASK-2.18 ✅: Yasal beyanları koruyan **ilk kapı** kuruldu — `tests/legal-consistency.test.ts`, **8 dal / 24 test**, `npm test` 180 → **204** (dosya 8 → 9). Paket metni kopyalamaz, **ilişkiyi** doğrular: her dal cümleden kısa bir parça alıp o parçanın **tam bir kez** geçtiğini ölçer, sonra dayanağı ayrıca ölçer. Sekizinci dal task dokümanında yoktu, **DURUM'un Not bloğundan** geldi (Vercel bölge cümlesinin depo içi yarısı: `vercel.json`/`preferredRegion` yokluğu). **On iki negatif kontrolün on ikisi kırmızı verdi** ve ikisi kapının KENDİ fail-open'ını buldu: dal 4 göreli URL'li depo okumasını, dal 2 cast'li doğrudan izleyici çağrısını kaçırıyordu — ikisi de genişletildi. Sınama düzeneği de bir tur fail-open koştu (ANSI yüzünden boş çıktı), ona da pozitif çapa eklendi. **İki sınır bilerek çizildi:** anahtarın yetkisi komşu depodadır (dal 4 yalnız *"sitenin kodu okumuyor"* yarısını çiviler) ve adresin posta alması DNS olgusudur (2.20). Yan bulgu Gelen Kutusu'na düştü. Beş ölçüm taban birebir.
...
**Adım:** task
**İlerleme:** Kapsam tartışması ✅ · teknik araştırma ✅ · task yazımı ✅ (20 task, dokuz bulgu) · plan doğrulama ✅ · plan revizyonu ✅ (2026-09-23: bir task iptal, üç hedefli düzeltme) · task çalıştırma 17/19 (TASK-2.01 → 2.18 tamamlandı; TASK-2.03 ❌ iptal). Sıra TASK-2.19'da — kalan iki task yasal beyan kapısının çapraz depo dalı (2.19) ve MX kaydı (2.20). Yasal **metin** tarafı bitti, **depo içi kapı** kuruldu.
...
**Task:** TASK-2.19 — Yasal beyan testi, çapraz depo "12 ay" dalı (B-060 kapanır)
**Durum:** ⬜ Bekliyor
**İlerleme:** TASK-2.18 kapandı ve arşive gitti; yasal beyanların **depo içi** sekiz olgusu artık `tests/legal-consistency.test.ts`'te çivili (8 dal / 24 test, batarya 180 → **204**). Geriye **tek olgu** kaldı: "12 ay" saklama — mekanizması komşu depoda (`../Alpfitplus-website.v1/pocketbase/pb_hooks/lead_lib.js` → `RETENTION_MONTHS`), yani salt-okunur bağlama + env kapısıyla gelecek (araştırma kararı, `PHASE-2.md` → Seçilen Yaklaşımlar 4). **B-060 orada kapanır.** Sonra MX kaydı (2.20). Sıradaki adım `/devflow:run-task`.
**Not:**
- ⚠️ **Depo içi kapı KURULDU (TASK-2.18) — 2.19'un borcu tek olgu: "12 ay".** `tests/legal-consistency.test.ts` sekiz dal taşıyor: sorgu ölçüme gitmiyor (`data-exclude-search`) · form alanları ölçüme gitmiyor (`track()` yükü davranışla) · ürün görselinde gerçek kişi verisi yok (temizlik tablosu + denetimin **52 parça ve 13 baş harfinin tamamıyla** sondalanması) · site depodan okumuyor (ucun tüm `fetch` çağrılarının yöntem+URL kümesi) · ölçüm üçüncü tarafa gitmiyor (`src/`'in dış host kümesi dondurulmuş allowlist) · çerez/tarayıcı deposu yok · başvuru adresi `CONTACT.support`'tan geliyor (ve `legal.ts`'e **elle yazılmamış**) · form ucu için bölge sabitlenmemiş (`vercel.json`/`preferredRegion` yokluğu). TASK-2.17'de ölçülen **depo dışı** olgular (Hetzner/DE · Resend ABD + `eu-west-1` · Google MX · Umami şemasında IP sütunu yokluğu · erişim kaydında rotasyon yokluğu) **bilerek çivilenmedi** — kaynakları canlı sistemler; çapaları `legal.ts`'in Aktarım yorumunda duruyor.
...
- **Faz 2 kapsamı dokuz bulgu:** B-029 ✅ · B-018 ✅ · B-024 ✅ · B-011 · B-058 ✅ · B-034 ✅ · B-055 ✅ · B-060 · B-059'un onay-e-postası ayağı ✅ (yan kazanç B-040 ✅). Kalan: B-060 (yalnız 2.19 — depo içi dal 2.18'de kapandı), B-011 (2.20). ⚠️ B-024 kapandı ama **hukuki dayanak (KVKK m.9) ayağı B-008'de devam ediyor** — o hukukçunun işi, fazı kilitlemiyor.
...
### TASK-2.18 — Yasal beyanları koruyan ilk kapı: depo içi sekiz olgu çivilendi

**Durum:** ✅ Tamamlandı — 2026-09-23

**Detay:** `tasks/archive/TASK-2.18.md`

**Özet:**
- **`tests/legal-consistency.test.ts` açıldı: 8 dal / 24 test, batarya 180 → 204 (dosya 8 → 9).** Paket yasal metni **kopyalamaz, ilişkiyi doğrular** — her dal cümleden kısa bir parça alıp o parçanın ilgili dokümanda **tam bir kez** geçtiğini ölçer (0 da >1 de kırar), sonra dayanağı ayrıca ölçer. Sessiz geçmeye karşı üç katman: iki yönlü `claimOnce` · her taramanın **boş kapsam bekçisi** (dosya sayısı tabanı + korpusta bilinen bir nişan dizesi) · karar fonksiyonlarının **pozitif çapa sondası**. Sekizinci dal task dokümanında yoktu, DURUM'un Not bloğundan geldi (bölge cümlesinin depo içi yarısı) — task dokümanı plan anında, Not bloğu bir gün sonra yazılmıştı, yenisi kazandı.
- **On iki negatif kontrolün on ikisi kırmızı verdi — ve ikisi kapının KENDİ fail-open'ını buldu.** Dal 4 depo çağrılarını URL önekiyle süzüyordu, **göreli** bir okuma çağrısı süzgecin dışına düşüp yeşil kalıyordu; dal 2'nin yüzey deseni (`window.umami`) araya giren bir **TypeScript cast'i** yüzünden doğrudan izleyici çağrısını kaçırıyordu. İkisi de genişletildi (tüm `fetch` çağrılarının yöntem+URL kümesi · `umami…​.track(` çağrı deseni) ve dört biçimde ayrı ayrı kırmızı verdirildi. Sınama **düzeneğinin kendisi** de bir tur fail-open koştu — ANSI kodları temizlenmeden grep'lendiği için boş çıktı verdi, "hata yok" diye okunabilirdi; düzeneğe de pozitif çapa eklendi. Dal 3'ün sondası tek örnekten (**"Weekend"** — eski marka parçası, oysa dalın cümlesi gerçek **kişi** verisi hakkında) **52 parçanın ve 13 baş harfin tamamına** genişletildi.
- **İki sınır bilerek çizildi, bir yan bulgu düşürüldü.** (a) *"Anahtar yalnızca yeni kayıt oluşturabilir, var olan kayıtları okuyamaz"* — anahtarın **yetkisi** komşu depoda yaşar; dal yalnız ölçülebilen yarıyı (*"sitenin kodu okumuyor"*) çiviler, devralınan yarı **çivilenmedi**. (b) Başvuru adresinin gerçekten posta alması DNS olgusudur, `npm test` ağ çağrısı yapmaz → 2.20. İkisi de kapının `ÖLÇÜLEMEYEN` yorum bloklarında yazılı. **Yan bulgu:** o cümlenin *"yalnızca oluşturabilir"* yarısı, ucun aynı anahtarla var olan kayda **PATCH** attığı gerçeğinden dar — metin düzeltmesi kapsam dışı, Gelen Kutusu'na düştü.

**Test:** `npm test` **204 geçti + 1 atlandı** (taban 180 + 1). `docker compose exec web npx tsc --noEmit` çıkış 0. Üretim derlemesi `docker compose build web-prod` çıkış 0. **Kaynak dosyalarda hiçbir değişiklik yok** — `git diff HEAD -- src/ research/` boş; on iki negatif kontrol kırılmasının tamamı scratchpad yedeğinden geri alındı ve **md5 ile doğrulandı** (`git checkout`/`git restore` kullanılmadı). **Beş ölçüm** (render edilen yüzey HEAD ile birebir aynı olduğu için rakamlar TASK-2.17'nin tabanını bağımsız olarak doğruluyor): `a11y` 8 rota **TOPLAM SORUN 0** · `mobile-audit` **9/9 yatay kaydırma yok**, dokunma hedefi **157** (taban birebir) · `font-guard` 16 sayfa / **85.015** karakter (taban birebir), eksik karakter yok · `scan` 390×844 `/kvkk` 9 kare / 7.373 px, **konsol temiz** · `perf` (3100) CLS ≤ 0,001, LCP 28-280 ms. **3100 bayatlık sondası:** üretim konteyneri TASK-2.17'nin üç yeni cümlesini de döndürüyor (1/1/1 vuruş), yani ölçülen yüzey güncel — varsayılmadı, ölçüldü.
...
**Aktif Task:** `tasks/TASK-2.19.md` ⬜ — Yasal beyan testi, çapraz depo "12 ay" dalı (B-060 kapanır)

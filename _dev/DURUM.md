...
**Son Güncelleme:** 2026-09-24 — **TASK-3.14 ✅ 320 px'te kesilen içerik kapandı: `Button` tabanı `sm:` ile sınırlandı, `FounderProgram` ızgarası `min-w-0` aldı.** Kapı `TOPLAM SORUN` **285 → 266**, @320 kırpılmış metin **19 → 0**. Devralınan teşhis **eksik çıktı**: kırpmayı bitiren `min-w-0` değil `nowrap`'in kalkmasıydı (kart min-content 370 → 222); `min-w-0` tek başına kırpmayı 0'a indiriyor ama etiketi kutudan 19 px taşırıyor. Sınır ölçülerek seçildi — `sm:` ≥ 640 px'te bugünkü hâli **0 farklı piksel** ile koruyor (4 sayfa / 40 M piksel). Yan bulgu: 320 px'te 1 px'e ezilmiş WhatsApp ikonu 16,31 px'e döndü.
...
**Adım:** task
**İlerleme:** Kapsam tartışması ✅ · teknik araştırma ✅ · task yazımı ✅ · plan doğrulama ✅ · **task çalıştırma 🔄 (14/25)** — keşif ayağı kapandı (TASK-3.01 · 3.02, ikisinde de plan revizyonu gerekmedi), **kapı kümesi (3.03-3.08) tamamlandı**: zemin kuruldu (3.03), kontrast dedektörü piksele taşındı (3.04), gradyan metin kendi dalını aldı (3.05), başlık hiyerarşisi ölçülmeye başladı (3.06), mobil kapı 320 px'i ve kesilen içeriği ölçmeye başladı (3.07), dokunma hedefi iki kulvara ayrıldı (3.08). **Düzeltme kümesi ilerliyor** (3.09-3.17, arada 3.25): ürün turunun soluk kartları AA'ya çıktı ve kapı 57 → 46'ya düştü (3.09), kapanış çağrısı bandı AA'ya çıktı ve kapı 46 → 26'ya düştü (3.10), gradyan metnin durakları AA'ya çıktı ve kapı 26 → 9'a düştü (3.11), desenli zeminin alt yazısı AA'ya çıktı ve kapı 9 → 8'e düştü (3.12), 404'ün dev rakamı dekoratif oldu ve başlık hiyerarşisi 16 sayfada düzeldi, kapı 8 → 6'ya düştü (3.13), 320 px'te kesilen 19 metin düğümü kapandı ve mobil kapı 285 → 266'ya düştü (3.14 — kontrast kulvarından **mobil kulvara** ilk geçiş). Her düzeltme kendi kalemini yeşile çeviriyor.
...
**Task:** **TASK-3.15** — Roller: sekme şeridi sığar, görsel eşlemesi düzelir · `tasks/TASK-3.15.md`
**Durum:** ⬜ Bekliyor — düzeltme kümesinin yedincisi
**İlerleme:** 14 / 25
**Not:**
...
**Aktif Task:** `tasks/TASK-3.15.md` ⬜ (düzeltme kümesinin yedincisi — Roller sekme şeridi); son kapanan `tasks/archive/TASK-3.14.md` ✅

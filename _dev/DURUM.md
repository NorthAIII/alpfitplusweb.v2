...
**Son Güncelleme:** 2026-09-24 — **TASK-3.07 ✅ mobil kapı artık 320 px'i de geziyor ve kesilen içeriği ölçüyor:** B-030'un (d) kalemi kapandı — dosya başlığı bir zamanlar "taşan metin" sayıyordu, kod ölçmüyordu. Devralınan rakam **birebir** çıktı: 320 px'te **19 gerçek kırpılmış eleman / en ağır 70 px**, 390 px'te **0**; muafiyet aritmetiği de tuttu (ham 78 = 19 gerçek + **59 muaf**, araştırmanın "muafiyetsiz 59 sahte pozitif" rakamıyla aynı). Tanım üç aday yan yana koşularak seçildi (metin menzili 9/66 · **doğrudan metin taşıyan elemanın kutusu 19/70** · tüm eleman 47/2595). Dört muafiyet de kuruldu ve ayrı sayılıyor; kaydırılabilir şerit ölçütü Roller şeridini yakaladı. Kapı: `TOPLAM SORUN 278 → 585`, çıkış **1**; belirlenimlilik birebir (433 satır). Beş sonda (kırmızı · **yeşil, çıkış 0** · boş sayfada sorun 0 olduğu hâlde dört kapsam eşiği · tablo muafiyeti · `aria-hidden` kalkınca üç düğüm geri sayıldı) ve gerçek sitede `min-w-0` deneyi **19 → 0 → 19**.
...
**Adım:** task
**İlerleme:** Kapsam tartışması ✅ · teknik araştırma ✅ · task yazımı ✅ · plan doğrulama ✅ · **task çalıştırma 🔄 (7/25)** — keşif ayağı kapandı (TASK-3.01 · 3.02, ikisinde de plan revizyonu gerekmedi), **kapı kümesi ilerliyor**: zemin kuruldu (TASK-3.03), kontrast dedektörü piksele taşındı (TASK-3.04), gradyan metin kendi dalını aldı (TASK-3.05), başlık hiyerarşisi ölçülmeye başladı (TASK-3.06), mobil kapı 320 px'i ve kesilen içeriği ölçmeye başladı (TASK-3.07); kapı kümesinin son task'i dokunma hedefini getiriyor (TASK-3.08), sonrasında düzeltmeler başlıyor.
...
**Task:** **TASK-3.08** — Dokunma hedefi: kritik küme kırmızı, gezinme yüzeyi raporlanır · `tasks/TASK-3.08.md`
**Durum:** ⬜ Bekliyor — kapı kümesinin sonuncusu; ölçen kurulur, düzeltme TASK-3.17'nin işi
**İlerleme:** 7 / 25
**Not:**
- ✅ **Kırpılmış taşma ve kaydırılabilir şerit artık ÖLÇÜLÜYOR; kapı 320 px'i de geziyor** (TASK-3.07). Ölçüt: **doğrudan metin taşıyan elemanın** kutusu, onu **kesen** atasının kutusunun dışına taşıyor mu — kesen kutu dıştan içe ilk daraltan atadır ve sınıfı o belirler. Devralınan rakam birebir çıktı: **320 px'te 19 gerçek / en ağır 70 px, 390 px'te 0** (hepsi `/` rotasında `FounderProgram`'da, kesen `<section>`). Dört muafiyet de kuruldu ve **ayrı sayılıyor**: ekran dışı beyanlı (bal küpü, 3) · görsel gizli (`clip-path: inset(50%)`, bugün 0) · hareketli şerit (19/18) · iki boyutlu içerik (`<table>`, 2); beşinci kova kaydırılabilir (40/38). İki yeni dalın kendi kapsam tabanı var (`BEKLENEN_METIN_ELEMANI = 2038` · `BEKLENEN_SERIT = 5`).
- ⚠️ **TASK-3.08 için ölçülmüş iki sınır.** (a) **Küçük dokunma hedefi sayısı iki katına çıktı** — 320 px de gezildiği için 275 → **550** (her genişlikte 275, yani sayı genişlikten bağımsız); kritik/gezinme ayrımı yapılmadığı için hepsi bugün `TOPLAM SORUN`'a giriyor ve bu ayrımı kurmak tam da TASK-3.08'in işi. (b) **Görüş alanı dışına kaymış öğe de ölçülebilir görünür** — yatay kaydırılabilir şeritte kayan kartın kesişim alanı 0'dır ama `getBoundingClientRect()` tam boyutu verir; hedef sayarken "görünür mü" sorusu alana bakarak yanıtlanamaz (ölçüldü, `memory/arastirma-konteynerinde-tarayici-olcumu.md`).
...
- 🔴 **Kapılar artık KIRMIZI koşuyor ve bu beklenen sonuçtur:** `a11y` 16 sayfada **57 sorun** (39 kontrast + 17 gradyan metin + 1 başlık atlaması; alt/h1/adsız link-buton hepsi 0), `mobile-audit` **2 genişlik × 16 sayfada 585** (550 küçük dokunma hedefi + 19 kırpılmış eleman + 16 şerit ihlali; yatay kaydırma ve taşan eleman her iki genişlikte de 0) — ikisi de çıkış kodu 1. Her düzeltme task'ı kendi kalemini yeşile çevirir; CI olmadığı için kırmızı hiçbir şeyi bloke etmez. **Yeşil ayağın çalıştığı her iki kapıda da ayrıca ölçüldü** (temiz hedefte `✓ KAPI YEŞİL`, çıkış kodu 0), yani kapılar kilitlenmiş değil.
...
**Aktif Task:** `tasks/TASK-3.08.md` ⬜ (kapı kümesinin sonuncusu — dokunma hedefi); son kapanan `tasks/archive/TASK-3.07.md` ✅

...
| 2.06 | TASK-2.06 | ✅ Tamamlandı | Hatalı alan kendi üstünde görünür: `aria-invalid` alanı 2 px `neg` halka + `neg-wash` zemin alıyor, hata metni alanın altında. Kontrol gruplu ölçüm (önce = 3100'deki TASK-2.05 imajı): geçersiz↔geçerli alan farkı **0/16 → 8/8**, alan düğümü görünür **0/36 → 28/28**, `aria-describedby` kendi düğümünü gösteriyor **0/36 → 28/28**, yanlış alarm işareti **16/24 → 24/24**; odak tablosu 24/24 korundu. `aria-invalid:` varyantı Tailwind 4.3.3'te yerleşik değil — `globals.css`'te `@custom-variant` ile kaydedildi. **B-055 kapandı ve arşive taşındı** |
| 2.07 | TASK-2.07 | ✅ Tamamlandı | Talep sahibine onay e-postası açıldı (metin `src/content/mail.ts`, dönüş süresi vaadi formunkiyle aynı, düz metin, `reply_to` ekip kutusu) ve `notify_lead` gerçek sonucu taşıyor. Yerel depoya karşı gerçek tur: iki e-posta da **`delivered`** (damgalar 214 ms arayla), uç 575 ms, kayıt `notify_lead: sent`; üç değerin üçü de (`sent`/`skipped`/`failed`) gerçek depo hook'una yazıldı — v2 artık `pending` kayıt bırakmıyor. Batarya 66 → 72 test, kontrol grubu tabanda 7 kırmızı verdi. **B-059'un iki ayağı kapandı**, atom açık |
| 2.08 | TASK-2.08 | ✅ Tamamlandı | Yetenek/yol haritası tek kaynağı kuruldu: `product.ts` → `CAPABILITIES` (`simdi` 12 · `yolda` 7 · `sonra` 5), kalemler `{id, label, modul?}` — çağrı yeri kalemi adıyla çağırıyor. **B-029'un beş karşılıksız iddiasının hiçbiri "bugün var"da değil**, beşi de "yolda"da ve her biri ürünün kendi erteleme kaydına dayanıyor (W8 · v1.5 · ertelendi). `PRODUCT_STATUS.modules` türetilir oldu; cümle anlamca aynı, **bir kalem düzeltildi** ("antrenör performansı" eklendi — ürün kodunda karşılığı ölçüldü; "Üye 360" bilinçle dışarıda). `short` silindi (0 tüketici), `version` kaldı (tüketicisi 2.10'da). Kapı `tests/capabilities.test.ts`, batarya 72 → 95; iki sondayla kırmızı görüldü. **Tüketiciler bağlanmadı** — sabiti atlayan 17 çağrı satırı / 6 dosya ölçüldü, 2.10+2.11'in kapanış ölçütü |
| 2.09 | TASK-2.09 | ✅ Tamamlandı | Beş cümleden **dördü düzeltildi, biri ölçümle çürütüldü**. Düzeltilen: Üye 360 artık ölçüm grafiği + diyetisyen notunu "tek ekranda" saymıyor (ikisi de üründe var ama başka ekranlarda); iptal eşiği "siz belirlersiniz" → "üründe sabit bir kural" (**24 saat yazılmadı** — çapasız iddia doğururdu); üyelik bitişi "bildirim gider" → "panelde listelenir" (giden gerçek bildirim `sendComebackT2`); "kampanya" modül metninden çıktı ve **`/ozellikler`'in iç çelişkisi kapandı** (servisten ölçüldü: eski ifade 0). Çürütülen: "yetkiler … geri alınır" **doğruymuş** — geri alma üretimde `revokeTemplate` üzerinden koşuyor (`accounts-update.ts:861`, uç `PATCH /accounts/:userId`); B-029 yalnız `revokeGrant`'e bakmıştı. Cümle korundu, `yolda` kalemi "tek yetkinin şablon değiştirmeden geri alınması"na daraltıldı. Yeni kapı `tests/iddia-metinleri.test.ts` (ziyaretçiye görünen yüzeyler; sabit kapısı bu sınıfı göremiyor — sondayla kanıtlandı), batarya 95 → **115** |
| 2.10 | TASK-2.10 | ✅ Tamamlandı | Yol haritasının **iki bileşen-içi kopyası kalktı**: `/ozellikler`'in üç kolonu ve Kurucu Programı'nın üç durum satırı `CAPABILITIES`'ten okuyor. B-040 kanıt komutu **15 → 10 satır / 6 → 4 dosya**, iki hedef ev **0**; kalan 10'un 3'ü sabitin kendisi, 7'si 2.11'in kapsamı. Serviste ölçüldü: `/ozellikler` **24 kalem = 12+7+5**, sabitle sayı ve sıra birebir; *"Kurumsal üyelik"* artık iki yüzeyde de var. Bağlamanın ikinci etkisi **görünürlük**: `yolda` 3 → 7 çıkmıştı ama site 3'ünü gösteriyordu, artık B-029'un dört kalemi ziyaretçiye "yolda" olarak görünüyor. **Devralınan `nextVersion` devri ölçümle çürütüldü** — ürünün sürüm haritası (`VERSIONS.md`, source of truth) v1.5'e yalnız üç kalem koyuyor, `yolda`'ya taşınan dört B-029 kalemi haritada yok; yedisine "v1.5" demek çapasız iddia olurdu. Alt iki başlık `STAGE_LABEL`'dan, alan **açılmadı**. Yeni ortak sabit `CAPABILITY_STAGES`; kapı `capabilities.test.ts`'e tüketici ayağı olarak eklendi, batarya 115 → **121**, iki sondayla kırmızı görüldü |
| 2.11 | TASK-2.11 | ✅ Tamamlandı | Yol haritasının **son dört evi** bağlandı: chat ağacı, SSS, `/fiyat` `NOT_INCLUDED` ve karşılaştırma sayfası `CAPABILITIES`'ten okuyor. Kanıt komutu **10 → 3 satır / 4 → 1 dosya**, kalan üçü sabitin kendisi → **B-040 kapandı ve arşive gitti**. `chat.ts` ilk kez `site.ts`'i import ediyor; pilot cümlesinin sabit dışında kopyası **0** → **B-014 de kapandı**. Tutarlılık kazancı: chat/SSS 3+3 kalem sayarken `/ozellikler` 7+5 sayıyordu, dört yüzey artık aynı listede (chat modül sayımı 8 → 9). Tekil cümlelerin **7'sinden 6'sı birebir aynı** render ediliyor. **Yeni yayın kapısı `upcomingCapability`/`stageNote`** ters yönü kapatıyor — kalem `simdi`'ye taşınırsa modül yüklenmiyor (sondada üç rota **500**). **Altıncı cümle ölçümle çıktı:** B-040'ın grep'i harfe duyarlı, `karsilastirma.ts`'in küçük harfli *"online ödeme"*'sini görmemişti. Batarya 121 → **154**, üç sondayla kırmızı görüldü. **Kapanmayan ayak:** `segments.ts`'te yol-haritası işareti hâlâ 0 → B-029/TASK-2.12 |
| 2.12 | TASK-2.12 | ✅ Tamamlandı | Riskli alt küme taraması koştu: konu sözcükleri ürünün erteleme notlarından **ve kanonik sürüm haritasından** (`VERSIONS.md` → v1.5/v2 adayları) türetildi — **27 küme**, 66 dosya, **129 benzersiz vuruş satırı** (98'i ziyaretçiye görünen), hepsi üçe ayrıldı. **İki karşılıksız iddia düzeltildi:** rapor filtresinin "tarih aralığı" vaadi → "ay" (ürün tek ay seçtiriyor — katalog + backend `month` + `<input type="month">`; şube ayağı doğru olduğu için korundu) ve antrenör görselinin alt metnindeki "öğrenci tutma" (kelime tüm ürün kod tabanında **0**, kalem v1.5'e taşınmış). Yeni kalem açılmadı — aralık zaten `yolda` → `gelismis-raporlama` kapsamında. **Yöntem sondayla düzeltildi:** ilk sürüm iki kavramı aynı satırda arıyordu ve task'ın adıyla istediği iki çapayı hiç görmemişti (21 vuruş); tek kavrama indirilip **çapa sondası** eklendi → 121 vuruş, 3/3 çapa. **B-029'un 2. ayağı çürütüldü:** `segments.ts`'in işaretsizliği boşluk değil **doğru sonuç** (36 parça × 12 anahtar → 0 gerçek vuruş). **B-029 kapandı ve arşive gitti**; kapanış kaydı taranmayan yüzeyi de yazıyor. Kapı `iddia-metinleri.test.ts`'e 6 senaryo olarak eklendi (yeni dosya yok), batarya 154 → **160**, iki sondayla kırmızı görüldü. Betik kalıcılaşmadı (M6 F6.4) |
| 2.13 | TASK-2.13 | ✅ Tamamlandı | B-018'in **dört** kalemi de görüntüden kalktı: "Gizem Ö." → "Yasemin U." (kaçıran şey tablo değil **kalıptı** — eşleme tam ada yazılmış, metinde kısaltılmış hâl geçiyordu), "Kampanyalar" **ortak kabuk kuralı** olarak yazıldı (`SHELL_DROP_NODES`) ve **7/7** ekranda tam bir düğüm düşürdü, "Yenileme & Churn" düştü (ızgara 6 → 5), ve TASK-2.12'nin devrettiği dördüncü kalem **"Öğrenci Tutma"** kartı düştü. Hat yeşil (7 `.webp`, çıkış 0), yedi çıktı gözle okundu; toplam 275.172 → **264.962 B**, boyutlar değişmedi. **Devralınan alt metin çürütüldü:** TASK-2.12 antrenör alt metnini *"haftalık doluluk ve ciro kırılımı"* yapıp `toContain` ile çivilemişti — o iki kart görüntüde **TASK-14.06'dan beri yok** ve ikisi de ürün kodunda reddedilmiş; bir karşılıksız iddia ikisiyle değiştirilmişti. Kapı elle listeden hattın **kendi düşürme tablosundan türeyen** kurala çevrildi. **Düşürme kendi kendini doğruluyor** (ölü izin satırı türetilerek çıkarıldı; kural kalkarsa üretim durur — sondayla, çıkış 1). ⚠️ Boş-kapsam sondası sınırı verdi: denetim üç kalemden **1'ini** görüyor ("Kampanyalar" tek sözcük, "Yenileme & Churn" `&` yüzünden kör) — **atom açık**, kök neden TASK-2.14/2.15'te. Batarya 160 → **163**, dört sondayla kırmızı görüldü |
| 2.14 | TASK-2.14 | ✅ Tamamlandı | Denetimin ad dalı `REPLACEMENTS`/`INITIALS`'ın **kaynak** tarafından türüyor (52 ad parçası + 13 avatar baş harfi; elle liste yok). Tablonun **hedef** tarafı çıkarılıyor — denetim kendi çıktısını sızıntı sayamaz: çıkarma olmasa `Plus` ⊂ "Alpfit Plus" yedi ekranı birden kırmızıya çekerdi. Ad parçası alt dize, baş harfi tam jeton (`SA` ⊂ `SAHİL` kapıyı kullanılamaz kılardı). **Kalıp kaldırılmadı, ikincil kaba ağa indirildi** — tabloya hiç girmemiş adı yalnız o dal görür ve TASK-2.13'ün kendi kendini doğrulayan kapısı ona dayanıyor; `AUDIT_ALLOW` yalnız onu kapatıyor. **İki devralınan varsayım ölçümle düzeltildi:** `INITIALS` kaynağı olduğu gibi yasaklanamıyordu (`takvim.html:166`'daki `EK` hedefin kendi baş harfi — "Ege K."), ve denetimin gördüğü kütle **bayattı** (avatar senkronu öncesi 6 baş harfi, sonrası 0) → toplama mutasyonların sonuna alındı. İki boş-kapsam tabanı kondu. Kontrol gruplu sonda: B-018'in tarihsel tablosunda eski denetim çıkış **0** + sızıntılı `grup.webp`, yeni denetim çıkış **1**, dosya yazılmadı. Çıktı 7/7 birebir aynı (denetim değişti, temizlik değil); batarya 163 → **171**, beş sondayla kırmızı görüldü |

...
| 2.20 | TASK-2.20 | ⏸️ Duraklatıldı | KVKK başvuru adresi posta alır (B-011). Yönerge yazıldı (Squarespace **Add preset → Google Workspace MX**, kaynağından doğrulandı — kararlaştırılan beş kayıtla birebir). Ölçüldü: apex MX hâlâ NODATA (iki çözümleyici), referans küme `kiwiailab.com`'da 5/5, TXT/NS/SOA tabanı alındı, apex A `76.76.21.21` → bugünkü posta **örtük MX** ile web IP'sine düşüyor. Kalan iki ayak kullanıcının DNS adımına bağlı — planlı el değişimi |

...
- **B-034 sınıfı gerçekten tek örnek, ama mekanik kural yanlış kurulursa beş yanlış alarm verir.** Bulgunun koruma önerisi "kırılım öneki taşımayan `flex-1`'i `flex-col` kabında ara" diyor; ölçüldü (verify-plan 2026-09-22'de yeniden sayıldı): depoda dokuz `flex-1` kullanımı var, yedisi kırılımsız ve **beşi meşru** (`SegmentsGrid.tsx:50,54` kart gövdesi yüksekliği · `Assistant.tsx:167` akış alanı · `ProductStory.tsx:200` satır kabında genişlik · `Assistant.tsx:150` satır kabında genişlik). Ayırt edici imza dar: **sabit yükseklik sınıfı (`h-*`) + `flex-1` + kolon kabı**. Kural bu imzayla yazılmazsa gürültü üretir.
...
- **B-034 düzeltmesi iki satırdır** (`flex-1` → `sm:flex-1`, `PriceCalculator.tsx:164,167`); mekanik kural eklenecekse imza "sabit yükseklik + `flex-1` + kolon kabı" olur, yalnız "kırılımsız `flex-1`" değil (beş yanlış alarm ölçüldü).

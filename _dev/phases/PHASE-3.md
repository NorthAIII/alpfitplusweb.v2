...
>
> **Bölme çocuğu:** `PHASE-3-UAT.md` — 38 senaryonun tam tablosu, otomatik kontrol dökümü (CI/bot/güvenlik taraması, faz penceresi `6f4eca9..HEAD`) ve kapıların kör noktaları (uat).

**Tarih:** 2026-09-26
**Toplam Senaryo:** 38 | **Geçen:** 28 | **Kalan:** 10

**Ölçüm yüzeyi:** beş ölçümün dördü + bu turun UAT sondası **yayın kopyası (3100)**; `scan.mjs` **geliştirme sunucusu (3000)** — betikte sabit, yani *"konsol temiz"* satırı yayın kopyası için hiç ölçülmedi. **3100 tazelenmedi, gerekmedi ve ölçüldü** (`lastmod` 2026-09-25T01:05:55.970Z = `632f99e`; sonraki üç commit `src/`+`public/`'e dokunmadı; pozitif kontrol `-sm.webp` 68, negatif kontrol 0).

**Beş ölçümün sonucu:** `mobile-audit` ✅ **TOPLAM SORUN 0 · çıkış 0** (kritik dokunma hedefi 305/0, gezinme kulvarı 333/261 — raporlanır, düşürmez) · `font-guard` ✅ **iki dal · çıkış 0** (85.129 karakter · 765/765 kesin ölçüm · muaf 10) · `perf` ✅ (`/` 111 KB / LCP 80 ms, çizgi 144 KB / 96 ms; 8 kombinde CLS 0) · `scan` ✅ konsol temiz **ama 3000'e karşı** · `a11y` ❌ **6 sorun · çıkış 1**. Batarya **219 geçti + 2 atlandı**, `tsc` 0, `lint` 30 (yeni yok), `npm audit` 0 açık, Vercel 20/20 `● Ready`.

**Kalan on senaryo — üç sınıf, hiçbiri ölçüm arızası değil:**

- **İki gerçek kusur → düzeltme task'ı** (kapsam-içi; gerekçe Task Listesi'nin altındaki notta): `/gecis`'in **altı kontrast kalemi** (`01`/`02`/`03` p02 **1,21** · "Elle tutulan kayıtlar…" **3,25** · `1` **3,49** · `2` **4,23**) → **TASK-3.26** ✅ — ⚠️ *koşuldu 2026-09-26: beşi kapandı (kapı 6 → 1), altıncısı ölçüm artefaktı çıktı ve kullanıcı kararıyla B-063'e bırakıldı; senaryo 3 yeniden koşumda `1 · çıkış 1` ile ve **bilinçli** olarak kırmızı kalacak* · hareket azaltma açıkken **çapa kaydırması hâlâ animasyonlu** (`html scroll-behavior: smooth` iki tercihte de; `reduce` bloğu yalnız `animation-*`/`transition-duration` sıfırlıyor) → **TASK-3.27** ✅ — ⚠️ *koşuldu 2026-09-26: çare tek satırdan ibaret ÇIKMADI.* CSS tarafı beklendiği gibi kapandı (`reduce` altında `scroll-behavior != auto` olan eleman 16 rota × 2 genişlikte **32 → 0**; gerçek yüzeyde asistan çapası **90 kare / 1.483 ms → 1 kare**, `/demo` sonuç kutusu **20 kare / 316 ms → 2 örnek / 16,5 ms**; `no-preference` üç ölçümde de **birebir** korundu). Ama tarama **ikinci bir yüzey** buldu: asistan akışı `behavior: "smooth"`u **JS'te açıkça** veriyor ve CSS sıfırlaması onu kapatmıyor — ölçüldü, kural enjekteyken de akış **39 kare / 633 ms** kayıyordu. O çağrı tercihi artık kendisi okuyor (`matchMedia`). Senaryo 24 yeniden koşumda yeşile döner; senaryo 23 gerilemedi.
- **İki kullanıcı kararı → task açılmadı** (kullanıcının kapattığı ya da henüz karar vermediği iş yeniden açılmaz): Roller'in **diyetisyen ayağı** ödünç görselde kaldı (TASK-3.24 ❌ İptal, 2026-09-25) · **ön-doldurulan kişisel verinin yasal metinde karşılığı yok** (KVKK → Aktarım dört tedarikçi sayıyor, WhatsApp yok; üç seçenek Gelen Kutusu `[TASK-3.25 SORU]`'da; site `noindex` önizlemede, **yayını bloke etmez ama yayından önce kapanmalı**).
- **Altı kalem `kanal: UAT`** — kullanıcının cihazı ya da gözü gerekiyor, alt ajan koşamaz: gerçek telefonla uçtan uca tur (Faz 2'den devredilen form denemesi dâhil) · iki yeniden tasarımın (Faydalar, Modüller) **görünüşünün beğenilmesi** — kullanıcı ikisini de hiç görmedi, geri dönüş çapaları `ae80fb0:src/components/sections/Benefits.tsx` ve `git revert c279aa1` · onay e-postası gelen kutusu/spam · ekran okuyucuda onay kutusunun iki kez duyurulması · `DEMO_TO` e-postasının yerleşimi · Umami panelinin **arayüzünde** v2 kaydı.

**Milestone hükmü — "kısmen":** dokuz kriterin yedisi ölçülerek karşılandı (320 px'te içerik/işlev kaybı yok · ilk ekranda dönüşüm yolu **9 eksende 16/16** · dönüşüm hedefleri ≥ 44 px · kapılar 16 sayfayı geziyor, gerçek rengi ölçüyor, ölçemediğini sayıyor ve eşik altında kırmızıya dönüyor · iki kart ızgarası reddedilen kalıptan çıktı · Roller eşlemesi düzeldi · yazı tipi kümesinin her karakteri dosyada bulundu). **İkisi açık:** *"ölçülmüş kontrast ihlali kalmadı"* — 6 kalem duruyor (TASK-3.26) — ve *"beş ölçüm yeşil"* — `a11y` kırmızı, `scan`'in yeşili ise yayın kopyasını hiç ölçmüyor. ⚠️ **TASK-3.26 sonrası (2026-09-26) ikisinin de hâli netleşti ve kapsam kararı VERİLDİ:** kapıdaki kalem **6 → 1** ve kalan 1 **sayfanın değil ölçümün kusuru** (gerçek değer 7,05; B-063). *"Ölçülmüş kontrast ihlali kalmadı"* kriteri **ürün tarafında karşılandı**. *"Beş ölçüm yeşil"* karşılanmıyor ve **karşılanmayacak**: kullanıcı kararı (2026-09-26, seçenek B) kapının `1 · çıkış 1` ile kapanmasıdır — betiğe dokunulmaz, B-063'ün ertelemesi bozulmaz, kalem açık kayıt olarak durur. Yani `review-phase` Adım 2 bu kriteri **"bilinçli olarak karşılanmadı"** diye hükme bağlayacak; kararın tam metni `tasks/archive/TASK-3.26.md` → Kapanış Gerekçesi. *"Roller sekmeleri doğru ekranı gösteriyor"* kriterinin diyetisyen ayağı verify-plan notunun öngördüğü gibi **açık** sayıldı. Hükmün nihai evi `review-phase` Adım 2'dir; emsal: Faz 1 *"milestone kısmen"*, Faz 2 *"milestone tam"*.

...
| 3.26 | TASK-3.26 | 🔄 Devam ediyor | **(UAT'tan doğdu)** `/gecis`'in altı kontrast kalemi — **beşi kapandı, kapı 6 → 1**; altıncısı ölçümün kusuru çıktı (B-063), kullanıcı kararı bekliyor |
| 3.27 | TASK-3.27 | ⬜ Bekliyor | **(UAT'tan doğdu)** Hareket azaltma açıkken çapa kaydırması animasyonsuz olur |
...
> ⚠️ **TASK-3.26 koşuldu (2026-09-26) ve "altı kalem" varsayımı ölçülerek BEŞE indi.** Beşi kapandı — üç dev rakam `aria-hidden` aldı (zemin üç genişlikte de saf beyaz, AX ağacında 0 düğüm, görünüş 0 piksel), iki küçük rakam ise **renkle değil rozetin zemini opaklaştırılarak** AA'ya çıktı (beş rakam birden 8,81). **Altıncı kalem sayfanın kusuru değilmiş:** `/gecis`'in *"Elle tutulan kayıtlar…"* paragrafının kendi kontrastı **7,05** ve zemini saf beyaz; kapıyı kırmızı tutan şey yapışkan başlığın **kendi gliflerinin** o paragrafın ölçüm maskesine sızması — **B-063'ün adı konmamış ikinci yüzü**, iki izolasyonla gösterildi. Naif yama ölçüldü ve yetmedi (6→5 ama gradyan tabanı 19→18, 8 eleman ölçüm dışı). B-063 **bilinçle ertelenmiş** bir borç (verify-plan 2026-09-23), yani kapının 0'a inmesi bu fazın kapsam kararını yeniden açar. Döküm: `tasks/TASK-3.26.md` → Oturum Kaydı · mekanizma `bulgular/B-063-*.md` + `modules/M6-Kalite-Kapilari.md` → Maske sızıntısı.
...
- **İki gerçek kusur → düzeltme task'ı** (kapsam-içi; gerekçe Task Listesi'nin altındaki notta): `/gecis`'in **altı kontrast kalemi** (`01`/`02`/`03` p02 **1,21** · "Elle tutulan kayıtlar…" **3,25** · `1` **3,49** · `2` **4,23**) → **TASK-3.26** — ⚠️ *koşuldu 2026-09-26: beşi kapandı (kapı 6 → 1), altıncısı ölçüm artefaktı çıktı ve açık; senaryo 3 yeniden koşmadan kapanmaz* · hareket azaltma açıkken **çapa kaydırması hâlâ animasyonlu** (`html scroll-behavior: smooth` iki tercihte de; `reduce` bloğu yalnız `animation-*`/`transition-duration` sıfırlıyor) → **TASK-3.27**.
...
**Milestone hükmü — "kısmen":** dokuz kriterin yedisi ölçülerek karşılandı (320 px'te içerik/işlev kaybı yok · ilk ekranda dönüşüm yolu **9 eksende 16/16** · dönüşüm hedefleri ≥ 44 px · kapılar 16 sayfayı geziyor, gerçek rengi ölçüyor, ölçemediğini sayıyor ve eşik altında kırmızıya dönüyor · iki kart ızgarası reddedilen kalıptan çıktı · Roller eşlemesi düzeldi · yazı tipi kümesinin her karakteri dosyada bulundu). **İkisi açık:** *"ölçülmüş kontrast ihlali kalmadı"* — 6 kalem duruyor (TASK-3.26) — ve *"beş ölçüm yeşil"* — `a11y` kırmızı, `scan`'in yeşili ise yayın kopyasını hiç ölçmüyor. ⚠️ **TASK-3.26 sonrası (2026-09-26) ikisi de daraldı ama kapanmadı:** kapıdaki kalem **6 → 1** ve kalan 1 **sayfanın değil ölçümün kusuru** (gerçek değer 7,05; B-063). Yani *"ölçülmüş kontrast ihlali kalmadı"* kriteri **ürün tarafında karşılanmış** sayılabilir; *"beş ölçüm yeşil"* ise hâlâ karşılanmıyor ve **kapıyı 0'a indirmek bilinçle ertelenmiş B-063'ü açmayı gerektiriyor** — hükmü `review-phase` Adım 2 verecek, ama kapsam kararı ondan **önce** gerekiyor. *"Roller sekmeleri doğru ekranı gösteriyor"* kriterinin diyetisyen ayağı verify-plan notunun öngördüğü gibi **açık** sayıldı. Hükmün nihai evi `review-phase` Adım 2'dir; emsal: Faz 1 *"milestone kısmen"*, Faz 2 *"milestone tam"*.

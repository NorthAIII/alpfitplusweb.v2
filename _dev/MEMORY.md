...
**Son Güncelleme:** 2026-09-26 — research-phase (Faz 4): *Vercel proje kimlikleri ve CLI erişimi* atomu gerçekle hizalandı, yeni dosya açılmadı. Kimlik yolu `263` → `266` (eski revizyonun dosyası duruyor ama token'ı geçersiz — ölçüldü) ve bulma tarifi `timeout` + boş stdin ile asılmaz hâle getirildi; REST için elle token okumak yerine `vercel api <yol>` yazıldı (env dizisinden değer basmama uyarısıyla). Index kancası aynı turda güncellendi.
...
- [Vercel proje kimlikleri ve CLI erişimi](memory/vercel-proje-kimlikleri.md) — **her `vercel` çağrısına çalışan kimlik yolunu `XDG_DATA_HOME` ile ver** (2026-09-26: `…/snap/code/266/…`; revizyon kayar, eskisinin token'ı geçersiz — bulma tarifi atomda), yoksa komut cihaz-giriş akışında asılır; REST için `vercel api <yol>`; `vercel env pull` `--sensitive` anahtarları **maskeli** getirir (gerçek gönderim gereken ölçüm anahtarı Vercel'den çekemez); proje `alpfitplus-web-v2` / takım `north-ai` / plan `hobby`

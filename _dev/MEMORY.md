...
**Son Güncelleme:** 2026-09-25 — TASK-3.22: *Tarayıcı ölçümü* atomuna **tek yeni bölüm**, yeni dosya açılmadı, index'e satır eklenmedi (kanca genişletildi). Kayıt: **`route.fulfill` CDP ağ kısıtlamasını BAYPAS EDER** — bu projenin sürekli kullandığı *"aday değeri kaynağa yazmadan enjekte ile ölç"* yöntemi **çizim ve stil** için sadık (aynı derlemede A/B, iki koşum, 0 fark), **zamanlama için değil**: 87 KB'lık LCP görselinin yerine 44 KB'lık aday enjekte edilince LCP 2672 → **1624 ms** okundu, gerçek değişiklik derlenip ölçüldüğünde **1800 ms** çıktı. Sadakat testi de yazıldı: ağdan bağımsız bir büyüklük (Σ|Δbölüm| = **1.670 px**) enjekte ve derlenmiş hâlde birebir çıkıyor, LCP çıkmıyor. ⚠️ Atom bu eklemeyle **20.636 token** oldu, yani kırmızı çizgiyi **aştı** — teşhis gerçek büyüme, ama 19 kardeş `##` bölümü arasından tek sonuç veren bir kesim olmadığı için **bölme kullanıcıya sorudur**; kap kararı verilmediği için KURAL yorumu yazılmadı, kalan doluluk `accept-size` ile kayda geçti.

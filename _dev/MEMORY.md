...
**Son Güncelleme:** 2026-09-25 — TASK-3.24: *Ürün iddiası çapa doğrulaması* atomuna **çapa kaymasının üçüncü biçimi** eklendi, yeni dosya açılmadı, index'e satır eklenmedi (kanca tek cümleyle genişletildi — bu bilgi dosya açılmadan bilinmezse oturum **doğru** bir devralınan iddiayı "çürüttüm" diye raporlar). Kayıt: **üretilen varlıkta çapa dizesi kaynakta hiç olmayabilir** — `render-product.mjs`'in temizlik tablosu gerçek ad/semt/markayı yeniden yazdığı için *"görüntüde şu yazıyor"* kanıtı karede doğru, kaynak HTML'de **0** olabilir; ölçüldü: T3.15'in `YÖNETİM` + `Zehra G. · Şube Müdürü` dizeleri kaynakta **0** kez geçiyor, karşılığı `class="ur">Kadıköy Müdürü`. Kural: çapayı hangi **katmanda** aradığını seç — kaynak ölçülüyorsa temizlik tablosunun girdi tarafı, çıktı ölçülüyorsa karenin kendisi.

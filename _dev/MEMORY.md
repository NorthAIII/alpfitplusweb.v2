...
**Son Güncelleme:** 2026-09-25 — TASK-3.21: iki mevcut atom genişletildi, **yeni dosya açılmadı, index'e satır eklenmedi**. (1) *Ürün iddiası* atomuna **8. kural**: **bir şeyi silmek de iddiadır ve "boş" bırakmak en sessiz iddiasıdır** — `alt=""` *"bu görsel bilgi taşımıyor"*, `aria-hidden` *"duyurulmasına gerek yok"* demektir; ölçüt **görünürlük değil** dört ayaktır (bağlantı içinde mi · üzerinde kendi metni var mı · `alt`ın içerik kelimelerinden kaçı sayfa metninde · varlığın cinsi), kapsam **çağrı yerine** ait olmalı, ve **reddedilen ölçüt kayda geçti** (piksel katkısı: zaten boş alt taşıyan bir bant kutusunun %99,89'unu değiştiriyor → ölçüt kontrol grubunu yanlış sınıflandırdı). (2) *Tarayıcı ölçümü* atomunun erişilebilirlik ağacı bölümüne: **`alt=""` de alt ağacı budar** (`ignored` olarak bile durmaz), yani `<img>` sayısı ile ağaçtaki görsel düğüm sayısı arasındaki fark üç kaynaktan gelir — `display:none` · boş alt · `aria-hidden` — ve **`opacity: 0` bu listede yoktur**. ⚠️ Tarayıcı ölçümü atomu **19.868 token**, kırmızı çizgiye **132 token** kaldı — sonraki ekleme onu aşar.

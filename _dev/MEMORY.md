...
**Son Güncelleme:** 2026-09-23 — TASK-2.18: iddia-çapası atomu genişletildi (**yedinci kural**), yeni dosya açılmadı. Yazdığın **kapı** da ölçülür ve onu ölçen **kontrolün kendisi** de: yeşil, kapının koştuğunun kanıtı değildir. Beş mekanik körlük sahada ölçüldü — dosya genelinde aranan desen **yorumları** sayar · **iki jetonlu** desen araya giren bir cast'le kör kalır · **önek süzgeci** aynı çağrının başka yazımını kaçırır · **tek örnekli sonda** hangi sınıfı ölçtüğünü seçemez ve bir ayağın körlüğünü başka bir ayak örtebilir · ⚠️ **negatif kontrol düzeneğinin kendisi** fail-open koşabilir (boş çıktı "hata yok" diye okunur).
...
- [Araştırma konteynerinde tarayıcı ölçümü](memory/arastirma-konteynerinde-tarayici-olcumu.md) — Playwright yalnız araştırma konteynerinde; betik scratchpad'e yazılıp `-v` ile mount edilir ve `--name` her koşumda farklı olur. ⚠️ **Mount noktası `/work`'ün içine düşerse repoya 0 baytlık root dosya bırakır** (ölçüldü) — betik `/work` dışına bağlanır; var olan bir dizinin (`product-out`, `lib`) üzerine bağlamak güvenlidir. ⚠️ **Bulamayan locator betiği yeşil bırakır** — üç ölçülmüş tuzak: aynı metin iki yerde · açık `role` niteliği rolü ezer · `next/image` `src`'i URL-kodlar (`img[src*="/product/"]` hiç eşleşmez); her koşumda "kaç eşleşme buldum" yazdır

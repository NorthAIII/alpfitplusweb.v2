...
**Son Güncelleme:** 2026-09-23 — TASK-2.13: iki mevcut atom genişletildi, yeni dosya açılmadı. (1) İddia-çapası atomu — **4. kural**: bir karşılıksız cümleyi silmek işin yarısı, **yerine yazdığın cümle de ölçülür**; onu elle yazılmış `toContain` listesiyle çivilemek koruma değil kilittir. Ölçülmüş hâl: TASK-2.12 *"öğrenci tutma"*yı doğru düşürdü ama yerine hattın **zaten düşürdüğü** iki kartı yazıp sabitledi; kapı artık hattın kendi düşürme tablosundan türüyor. (2) Tarayıcı ölçümü atomu — `next/image` `src`'i URL-kodladığı için `img[src*="/product/"]` **hiç eşleşmez** ve betik hatasız "bulunamadı" basar; ayrıca sekme→görsel eşlemesi birebir değil (`diyetisyen: SHOTS.antrenor`), varsayılmaz, `Roles.tsx` okunur.
...
- [Araştırma konteynerinde tarayıcı ölçümü](memory/arastirma-konteynerinde-tarayici-olcumu.md) — Playwright yalnız araştırma konteynerinde; betik scratchpad'e yazılıp `-v` ile mount edilir, `research/`'e yazılmaz ve `--name` her koşumda farklı olur. ⚠️ **Bulamayan locator betiği yeşil bırakır** — üç ölçülmüş tuzak: aynı metin iki yerde · açık `role` niteliği rolü ezer · `next/image` `src`'i URL-kodlar (`img[src*="/product/"]` hiç eşleşmez); her koşumda "kaç eşleşme buldum" yazdır

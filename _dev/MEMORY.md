...
**Son Güncelleme:** 2026-09-26 — TASK-3.27: *Araştırma konteynerinde tarayıcı ölçümü* atomunun **kaydırma yarışı** maddesi gerçekle hizalandı, yeni dosya açılmadı, index'e satır eklenmedi. Kayıt iki yönlü değişti: (a) sitenin `scroll-behavior: smooth`'u artık hareket azaltma altında **kapanıyor** (ölçüldü: `reduce` bağlamında `auto` olmayan eleman 16 rota × 2 genişlikte **32 → 0**), yani `reducedMotion:'reduce'` ile açılan ölçümde koşma gereksiz — ama `no-preference` ile açılan her bağlamda (mobile-audit · scan · perf) **hâlâ şart**; (b) yeni ve ölçülmüş bir sınır eklendi: **CSS sıfırlaması, `behavior`ı JS'te AÇIKÇA veren çağrıyı kapatmaz** (asistan akışı kural enjekteyken de 39 kare / 633 ms kaydı) — öyle bir çağrı tercihi `matchMedia` ile kendisi okumalı. Bayat satır numaraları da düzeltildi (`:135`/`:232` → `:203`/`:326`).

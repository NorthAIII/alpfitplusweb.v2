...
**Son Güncelleme:** 2026-09-24 — TASK-3.04: tarayıcı-ölçümü atomu iki yeni tuzak kazandı, **yeni dosya açılmadı, index'e satır eklenmedi** (kanca genişletildi). İkisi de ekran ekran gezen **her** ölçümü vurur ve ikisi de sessizce *sahte kırmızı* üretir: (1) sitenin `scroll-behavior: smooth`'u hareket azaltma tercihiyle kapanmadığı için DOM okuması ile ekran karesi farklı kaydırma konumunda alınabiliyor (ölçüldü: sayfa altındaki 25 eleman "ölçülemedi"); (2) stil değişikliği bir sonraki boyamaya kadar kareye girmiyor ve **kısmen** giriyor — kare hemen alındığında yalnız yapışkan başlığın kendi bileşke katmanı güncellenmişti (kare farkı 1.839 vs komşu adımda 69.016). Teyit ölçütü belirlenimliliktir: iki ardışık turun birebir aynı çıktıyı vermesi.

...
**Son Güncelleme:** 2026-09-24 — TASK-3.03: iki mevcut atom genişledi, **yeni dosya açılmadı, index'e satır eklenmedi**. (1) Kapı sınaması iki sonda daha kazandı — kapının **kapsamı** da eşiklenir (yoksa "hiçbir şey ölçmedim" ile "sorun bulmadım" aynı yeşili basar; B-030'un `font-guard` kalemi tam bu sınıftı) ve **yeşil ayağı** da sınanır (kırmızı kilitlenmesi ile gerçek ihlal ancak temiz bir hedefle ayrışır). (2) O sondaların düzeneği yazıldı: `research` `network_mode: host` taşıdığı için scratchpad'de duran statik bir site kapıya gerçek hedef gibi görünür — kaynağa değil girdiye dokunulur, sunucunun kapanması pozitif kontrolle ölçülür.

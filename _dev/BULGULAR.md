...
- [TASK-2.18] Yayındaki yasal metin *"sitenin kullandığı anahtar **yalnızca yeni kayıt oluşturabilir**, var olan kayıtları okuyamaz"* diyor (`legal.ts` → Aktarım), ama uç **aynı anahtarla** var olan kayda `PATCH` de atıyor (`route.ts` → `notifyStore`; komşu deponun kendi sözleşmesi de token'ın **iki** ucu açtığını yazıyor: `POST /lead` + `PATCH /lead/{id}`). Okuma yasağı ayrı bir olgudur ve çürütülmedi — dar olan *"yalnızca oluşturabilir"* yarısı: metin anahtarı olduğundan **kısıtlı** gösteriyor. Metin düzeltmesi TASK-2.18'in kapsamı dışıydı (kapı yazıldı, metin 2.16/2.17'de son hâlini almıştı); kapının dal 4'ü bilerek yalnız ölçülebilen yarıyı çiviliyor. Ölçüm: `tasks/archive/TASK-2.18.md` → Kararlar

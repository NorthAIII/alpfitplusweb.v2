...
**Son Güncelleme:** 2026-09-13 — TASK-1.11: Gelen Kutusu'na kullanıcı kararı satırı ve Bunker OS'a ait iki gözlem düştü; yedek çelişkisi satırı ölçümle çözüldü ve task kaydına mezun edildi. Açık bulgu 43 — rehber eşik (~30) aşıldı, triyaj çağrısı sürüyor.
...
- [TASK-1.11] Bunker OS'a ait (bu projede çözülmez): canlı n8n iş akışı "HERMES — Autonomous Loop (Daily)" → "Slack Notify" düğümünde Slack gelen-webhook adresi düz metin gömülü; değer günlük veritabanı dökümlerine de giriyor. Konum: `workflow_entity`/`workflow_history` düğüm parametresi (değer yazılmadı). Rota: Bunker OS'ta rotate + `$env`

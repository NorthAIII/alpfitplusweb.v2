...
- [ ] **3. Başlık testi** — `tests/guvenlik-basliklari.test.ts` (YENİ): `next.config.ts`'in `headers()` çıktısında üretim koşulunda politika v1 dizesiyle birebir ve `'unsafe-eval'` **yok**; geliştirme koşulunda var; `DENY` ve `Permissions-Policy` değerleri. Beklenen değerler `research/lib/gecis-envanteri.mjs`'ten okunur (TASK-4.03'ün tek evi; `web` konteyneri depoyu `.:/app` ile gördüğü için erişilebilir) — testte üçüncü bir kopya yazılmaz. (Tek komut/CI gelince bu test gerilemeyi kendiliğinden yakalar.)

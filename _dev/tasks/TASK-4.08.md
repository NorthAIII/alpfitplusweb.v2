...
v1'in `hreflang` alternatifleri tek dil kararıyla **yazılmaz**; `Organization.logo` v1'de de yok, parite dışı. Tamam sayılır: Vitest alanları ve türetimi çiviliyor; 3100'de blok 15 sayfada geçerli JSON ve alanlar yerinde.
...
- **`Organization.email`:** (a) `CONTACT.support` — `destek@alpfitplus.com`, sitenin yayındaki iletişim adresi; B-011 kapanana dek posta almaz, ama B-011 geçişten önce kapanır (TASK-4.15) ve yapılandırılmış veri ancak canlıda okunur. (b) `CONTACT.sales` — `kivanc@kiwiailab.com`, bugün posta alıyor ama kişisel bir adres kurumsal kayda girer. **Öneri (a).** Kullanıcıya sorulur.
...
- [ ] Elle yazılmış telefon dizesi taraması 0 ve taramanın pozitif çapası (türetilmiş değerin kaynakta bulunduğu) yeşil

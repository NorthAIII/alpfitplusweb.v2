...
- **Tablo satır sırası = çalıştırma sırası** (tablo = faz dokümanındaki Task Listesi; DURUM'un Task Durumu tablosu onu aynalar). Normalde numara sırasıyla örtüşür; plan revizyonu araya task soktuğunda numara sırası sapabilir — **tablo sırası esastır**
...
**Plan revizyonu gerekti (run-task oturumunda — task hatalı varsayımda, eksik ya da çelişen task var, ya da planlı bir keşif/doğrulama ayağı kalan task'ların doğruluğunu değiştiren bulgu üretti):**
→ Task yazma; DURUM Adım'ını `plan`'a çek ve oturumu kapat — plan-phase revizyon modu devralır. Rotanın tam mekaniği (kesilen task 🔄 mı, biten keşif ayağı ✅ + arşiv mi; hangi commit) **tek evde**: run-task → "Plan revizyonu gerekirse".
